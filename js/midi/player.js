...
midi.loadMidiFile = function(onsuccess, onprogress, onerror) {
	try {
		...
		for(var i = 0;i<midi.data.length;i++){
			if(midi.data[i][0].event.subtype == 'trackName'){
				// i++;
				// drums = midi.data[i][0].event.channel;
				console.log(midi.data[i][0].event.text);
			}

		}
		console.log(drums);
		// for(var i = 0;i<midi.data.length;i++){
		// 	if(midi.data[i][0].event.subtype == 'noteOn' && midi.data[i][0].event.channel == drums){
		// 		console.log(midi.data[i][0].event.noteNumber);
		// 	}
        //
		// }
